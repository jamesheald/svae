...
class LDS(LinearGaussianChainPrior):
    ...
    def sample(self, params, u, shape, key):
        ...
        obs_noise = tfd.MultivariateNormalFullCovariance(loc=d, covariance_matrix=R)\
            .sample(sample_shape=sample_shape, seed=key1)
        obs = np.einsum("ij,...tj->...ti", C, latents) + obs_noise
        # U, v, S = params["U"], params["v"], params["S"]
        # control_noise = tfd.MultivariateNormalFullCovariance(loc=v, covariance_matrix=S)\
        #     .sample(sample_shape=sample_shape, seed=key2)
        # controls = np.einsum("ij,...tj->...ti", U, latents) + control_noise
    ...
    def log_prob(self, params, u, states, data):
        ...
        # # Gets around batch dimensions
        # control_noise = tfd.MultivariateNormalFullCovariance(loc=v, covariance_matrix=S)
        # control_ll = control_noise.log_prob(u - np.einsum("ij,...tj->...ti", U, states))
        # return latent_ll + obs_ll.sum(axis=-1) + control_ll.sum(axis=-1)
    ...
    def marginal_log_likelihood(self, params, u, data):
        ...
        lps = joint_ll - posterior_ll # log p(x, y, u) - log p(x|y, u) = log p(x) + log p(y|x) + log p(u|x) - log p(x|y, u) = log p(y, u)
...
def sample_lds_dataset(run_params):    
    ...
    # U = jr.normal(seed_U, shape=(input_dims, latent_dims))
    ...
    u = u.transpose(0,2,1) *0.0# num_trials x num_timesteps x input_dims
    ...
    val_u = val_u.transpose(0,2,1) *0.0# num_trials x num_timesteps x input_dims
