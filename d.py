...
def svae_loss(key, model, data_batch, target_batch, u_batch, model_params, goal_obs, itr=0, **train_params):
    ...
    # MM_prior = {}
    # MM_prior['Ex'] = RPM_batch['mu'].mean(axis = 0)
    # mu_diff = RPM_batch['mu'] - MM_prior['Ex'][None]
    # MM_prior['Sigma'] = RPM_batch['Sigma'].mean(axis = 0) + np.einsum("ijk,ijl->ijkl", mu_diff, mu_diff).mean(axis = 0)
    # MM_prior['J'] = vmap(lambda S, I: psd_solve(S, I), in_axes=(0, None))(MM_prior['Sigma'], np.eye(MM_prior['Ex'].shape[-1]))
    # MM_prior['h'] = np.einsum("ijk,ik->ij", MM_prior['J'], MM_prior['Ex'])
    ...
    # prior_marg_params = {}
    # prior_marg_params["Ex"] = prior_params_batch["Ex"].mean(axis=0)
    # mu_diff = prior_params_batch["Ex"] - prior_marg_params["Ex"][None]
    # prior_marg_params['Sigma'] = prior_params_batch['Sigma'].mean(axis = 0) + np.einsum("ijk,ijl->ijkl", mu_diff, mu_diff).mean(axis = 0)
