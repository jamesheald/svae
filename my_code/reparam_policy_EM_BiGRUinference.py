...
def compute_free_energy_E_step(prior_params, prior_JL, J_RPM, mu_RPM, Sigma_RPM, smoothed, u, key, batch_id, options):
    ...
    Sigmatnt = -np.einsum("ti,tj->tij", smoothed['smoothed_means'][:-1], smoothed['smoothed_means'][1:])
...
def get_free_energy_E_step(params, prior_params, prior_JL, opt_states, y, u, RPM, key, options):
...
def one_E_Step(carry, inputs, options):
    ...
    params, prior_JL, opt_states, y, u, RPM = carry
    key = inputs

    (loss, (kl_qp, ce_qf, ce_qF)), grads = get_value_and_grad_E_step(params, params['prior_params'], prior_JL, opt_states, y, u, RPM, key, options)
    params, opt_states = params_update_E_step(grads, opt_states)

    carry = params, prior_JL, opt_states, y, u, RPM
...
def E_step(params, opt_states, y, u, key, options):
    ...
    (params, _, opt_states, _, _, _), (loss, kl_qp, ce_qf, ce_qF) = scan(partial(one_E_Step, options=options), carry, keys)
...
options['f_time_dependent'] = True
options['num_MC_samples'] = 10
...
print("should I be calculating the Sigmatnt cross entropy terms if q factorises across time?")
