...
class RPM:
    ...
    def elbo(self, key, data, target, # Added the new target parameter 
        ...
        # potential['J'] = RPM_batch["J"][batch_id] - prior_marg_params["J"]
        # potential['h'] = RPM_batch["h"][batch_id] - prior_marg_params["h"]
        # # potential['Sigma'] = vmap(lambda J, I: psd_solve(J + np.eye(3) * (abs(np.linalg.eigvals(J).min()) + 1e-4), I), in_axes=(0, None))(potential["J"], np.eye(self.prior.latent_dims))
        # potential['Sigma'] = vmap(lambda J, I: psd_solve(np.diag(np.clip(np.diag(J),a_min=1e-6, a_max=None)), I), in_axes=(0, None))(potential["J"], np.eye(self.prior.latent_dims))
        # potential['mu'] = np.einsum("ijk,ik->ij", potential['Sigma'], potential['h'])
        ...
        kl_qp, kl_qf, posterior_entropy = self.kl_terms(params, posterior, current_prior_params, u, normalised_auxillary_J[batch_id], normalised_auxillary_h[batch_id], delta_q)
        ...
        # kl_qf = self.kl_qp_natural_parameters(posterior_precision, posterior_precision @ posterior.expected_states.reshape(-1), block_diag(*(prior_J + RPM_batch["J"][batch_id])), (prior_h + RPM_batch["h"][batch_id]).reshape(-1))
        ...
        # rpm_Sigma = vmap(lambda S, I: psd_solve(S, I), in_axes=(0, None))(prior_J + RPM_batch["J"][batch_id], np.eye(self.prior.latent_dims))
        # rpm_mu = np.einsum("ijk,ik->ij", rpm_Sigma, prior_h + RPM_batch["h"][batch_id])

        # rpm_Sigma_all = vmap(vmap(lambda S, I: psd_solve(S, I), in_axes=(0, None)), in_axes=(0, None))(prior_J[None] + RPM_batch["J"], np.eye(self.prior.latent_dims))
        # rpm_mu_all = np.einsum("hijk,hik->hij", rpm_Sigma_all, prior_h[None] + RPM_batch["h"])

        # take samples under the posterior
        # num_samples = params.get("obj_samples") or 1
        # samples = self.posterior.sample(posterior_params, u, (num_samples,), key) # I HAVENT YET GONE FOR THIS CODE FOR CONTROLLED LDS CASE
        # log_f_prob = vmap(vmap(lambda m, S, x: MVN(loc=m, covariance_matrix=S).log_prob(x), in_axes=(0, 0, 0)), in_axes=(0, 0, None))(rpm_mu_all, rpm_Sigma_all, samples[0,:,:])
        # samples = MVN(loc=posterior.expected_states.reshape(-1), covariance_matrix=psd_solve(posterior_precision, np.eye(posterior_precision.shape[-1]))).sample(seed=key).reshape((rpm_mu_all.shape[1],rpm_mu_all.shape[2]))
        # log_f_prob = vmap(vmap(lambda m, S, x: MVN(loc=m, covariance_matrix=S).log_prob(x), in_axes=(0, 0, 0)), in_axes=(0, 0, None))(rpm_mu_all, rpm_Sigma_all, samples)
        # batch_size = rpm_mu_all.shape[0]
        # F_log_p = logsumexp(log_f_prob, axis=0, b=1/batch_size).sum()
        # kl_qF = -posterior_entropy - F_log_p
        ...
        # free_energy = policy_loss - kl_qp - kl_qf + kl_qF # + T_log_N - T_log_N
...
class RPMLDS(RPM):
    ...
    def kl_terms(self, params, posterior, prior_params, u, normalised_auxillary_J, normalised_auxillary_h, delta_q):
        ...
        return kl_qp, kl_qf, posterior_entropy
